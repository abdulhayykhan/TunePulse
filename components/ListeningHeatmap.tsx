...
export default function ListeningHeatmap({ heatmap, isLoading }: ListeningHeatmapProps) {
  ...
  return (
    <section className="rounded-[1.75rem] border border-white/5 bg-[#101010] p-6 shadow-glow">
      ...
      {isLoading ? (
        <div className="h-[520px] animate-pulse rounded-[1.5rem] border border-white/5 bg-white/[0.03]" />
      ) : (
                ...
                <div key={hour} className="grid grid-cols-[44px_repeat(7,minmax(0,1fr))] gap-2">
                  ...
                  {Array.from({ length: 7 }, (_, day) => {
                    ...
                    return (
                      <div
                        key={`${day}-${hour}`}
                        className="group relative h-6 rounded-md border border-white/5 transition hover:scale-[1.02]"
                        style={{ background }}
